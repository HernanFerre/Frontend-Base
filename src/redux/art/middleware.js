...
import { artByDescriptionFetch, artFetch, artByIdFetch, addArtFetch, updateArtFetch, deleteArtFetch } from "../fetchs";
...
export const deleteArt =
    ...
    (action) => {
        next(action);
        if (action.type === DELETE_ART) {
            dispatch(RESTDelete(deleteArtFetch, action.id, DELETE_ART_SUCCESS, DELETE_ART_ERROR, ""));
        }
    };

export const middleware = [getArt, getByDescription, getById, addArt, updateArt, deleteArt];
